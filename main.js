...
var db = new JsonDatabase("JsonData");
var yamldb = new YamlDatabase("myDatabase");

var ayar = {
  host: "MiecraftServerIp", //Sunucu IPnizi giriniz.
  port: 43878, //Sunucu portunuzu giriniz.
  username: "BotName", //Sunucuya giriş yapacak bot isminizi girin.
...
bot.on("chat", function(username, message) {
  ...
  if (kayit.authme == "var") {
    ...
    if (!giris) {
      ...
      if (automessage == true) {
        setInterval(() => {
          bot.chat("Botun Her 5 Dakkada Ne Atmasını İstiyorsanız Onu Yazabilirsiniz."); // değiştirmek çok basit '' arasındaki yazıyı değiştirin yeter
        }, 300000);
      }
    }
    if (giris) {
